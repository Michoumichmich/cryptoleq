...
Cell Expr::val() const {
    switch (typ) {
        case eTerm:
            return get<Term>(LNFUN, children[0])->val();
        case ePlus:
        case eMinus: {
...
Cell Term::val() const {
    switch (typ) {
        case eCnst:
            return get<Cnst>(LNFUN, children[0])->val();
        case eId: {
        ...
        case eExpr:
            return get<Expr>(LNFUN, children[0])->val();
        case eMinus: {
...
Cell Cnst::val() const {
    switch (typ) {
        case eTsnum:
            return get<Tsnum>(LNFUN, children[0])->val();
        case eChr:
            return Cell(int(s[0]), 0);
        case eQmk: {
...
Cell Term::func_comp(int x, Token k) const {
    ...
    switch (x) {
        case 1:
            r = testRnd(comp.peek_rndN(), k, root()->comp->proc.N);
            break;
        case 2:
            r = testRnd(comp.random(), k, root()->comp->proc.N);
            break;
        case 3:
            return testB2beta(Cell(comp.proc.B2, 0), k);
        case 4:
            return testB2beta(Cell(comp.proc.beta, 0), k);
        case 5:
            return Cell(testFkf(comp.fkf(), k), Cell::X);
        case 6:
            return Cell(comp.proc.N / 2, Cell::X);
        case 7:
            return Cell(comp.access_phi(), 0);
        case 8:
            return Cell(comp.access_g(), Cell::X);
        default:
            throw Err(LNFUN);
    }
